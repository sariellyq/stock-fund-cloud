...
-- V4.1 dynamic industry allocation table
create table if not exists fund_industry_allocations (
  ...
  weight numeric default 0,
  created_at timestamptz default now(),
  unique(fund_id, industry)
);

alter table fund_industry_allocations enable row level security;

drop policy if exists "allow all fund_industry_allocations" on fund_industry_allocations;
create policy "allow all fund_industry_allocations" on fund_industry_allocations for all using (true) with check (true);

-- Add default resource ETF if missing
insert into industry_etfs (industry, etf_code)
select '资源', '510410'
where not exists (select 1 from industry_etfs where industry = '资源');
