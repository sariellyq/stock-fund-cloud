...
def estimate_fund(fund):
    ...
    total_pnl = amount * fund_return / 100
    current_value = amount + total_pnl
    return fund_return, total_pnl, current_value, rows
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("专注盘中使用：股票只看当日5分钟K；基金按重仓股票当前涨跌幅估算。数据来自 Yahoo Finance，通常有延迟，仅用于估算，不构成投资建议。")
    ...
    with tab_fund:
        ...
        if not st.session_state.funds:
            ...
            if st.button("计算当前基金估算", type="primary"):
                ret, pnl, value, rows = estimate_fund(fund)
                c1, c2, c3 = st.columns(3)
                c1.metric("估算基金今日涨跌幅", f"{ret:.2f}%")
                c2.metric("估算盈亏", f"{pnl:,.2f}")
                c3.metric("估算当前金额", f"{value:,.2f}")
    ...
    with tab_manage:
        ...
        with st.expander("第一步：创建/选择基金记录", expanded=True):
            ...
            if st.button("创建基金记录"):
                if not fund_name:
                    ...
                    fid = str(uuid.uuid4())
                    st.session_state.funds[fid] = {"name": fund_name, "amount": fund_amount, "positions": []}
        ...
        if st.session_state.active_fund_id and st.session_state.active_fund_id in st.session_state.funds:
            ...
            with st.expander("第二步：添加重仓股票", expanded=True):
                ...
                total_weight = sum(float(p.get("weight", 0) or 0) for p in fund.get("positions", []))
                st.info(f"当前重仓合计：{total_weight:.2f}%")
