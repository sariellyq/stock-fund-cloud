...
def calc_industry_weighted_return(fund_id):
    weights_row = get_fund_industry(fund_id)
    weights = {
        "科技": float(weights_row.get("tech", 0) or 0),
        "制造": float(weights_row.get("manufacturing", 0) or 0),
        "消费": float(weights_row.get("consumption", 0) or 0),
        "金融": float(weights_row.get("finance", 0) or 0),
        "医药": float(weights_row.get("medical", 0) or 0),
    }
...
def main():
    ...
    st.title(APP_TITLE)
    st.caption("V4.0 Supabase 数据库版：刷新、关闭网页、重新部署后数据仍保存在数据库中。")
        ...
        with st.expander("基金管理", expanded=True):
            ...
            if st.button("创建基金"):
                if f_name:
                    ...
                    upsert_fund_industry(new_id, {"tech": 0, "manufacturing": 0, "consumption": 0, "finance": 0, "medical": 0})
                    st.success("基金已创建。")
                    st.rerun()
            ...
            if funds:
                ...
                indrow = get_fund_industry(fid)
                st.write("行业分布")
                i1, i2, i3, i4, i5 = st.columns(5)
                with i1:
                    tech = st.number_input("科技%", min_value=0.0, max_value=100.0, value=float(indrow.get("tech", 0) or 0), step=0.5)
                with i2:
                    manu = st.number_input("制造%", min_value=0.0, max_value=100.0, value=float(indrow.get("manufacturing", 0) or 0), step=0.5)
                with i3:
                    cons = st.number_input("消费%", min_value=0.0, max_value=100.0, value=float(indrow.get("consumption", 0) or 0), step=0.5)
                with i4:
                    fin = st.number_input("金融%", min_value=0.0, max_value=100.0, value=float(indrow.get("finance", 0) or 0), step=0.5)
                with i5:
                    med = st.number_input("医药%", min_value=0.0, max_value=100.0, value=float(indrow.get("medical", 0) or 0), step=0.5)
                ...
                if st.button("保存基金基础信息"):
                    ...
                    upsert_fund_industry(fid, {"tech": tech, "manufacturing": manu, "consumption": cons, "finance": fin, "medical": med})
                    st.success("已保存。")
                    st.rerun()
